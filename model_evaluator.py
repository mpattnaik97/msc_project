...
class Evaluator:
    ...
    def predict_on_dataset(self,
        ...
        for j in tr:
            # Generate batch.
            batch_X, batch_image_ids, batch_eval_neutral, batch_orig_labels, batch_image_sizes = next(generator)
            # Predict.
            ...
            for k, batch_item in enumerate(y_pred):
                ...
                original_sizes = np.array(batch_image_sizes[k], dtype=np.float)
                for box in batch_item:
                    ...
                    xmin = round(box[xmin_pred] * (original_sizes[1]/img_width))
                    ymin = round(box[ymin_pred] * (original_sizes[0]/img_height))
                    xmax = round(box[xmax_pred] * (original_sizes[1]/img_width))
                    ymax = round(box[ymax_pred] * (original_sizes[0]/img_height))
